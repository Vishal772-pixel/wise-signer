...
import TrezorScreens, {
  TrezorScreensProps,
  CHUNK_SIZE,
} from "@/components/wallets/TrezorScreens";
...
const WalletPopupComponent = ({
  ...
  if (isTransaction) {
    networkName =
      transactionOrSignatureDetails.networkName || "Ethereum Mainnet";
  }
  ...
  const totalTrezorScreens = (() => {
    if (isTransaction) {
      ...
      const signatureDetails =
        transactionOrSignatureDetails as SignatureDetails;
          ...
          <div className="flex items-center">
            <div className="mr-2 w-8 h-8 bg-orange-500 rounded-full flex items-center justify-center overflow-hidden">
              ...
              >
                ...
                />
                <path
                  d="M103.6 152.5L87.9 180.9L144.1 184.2L142.1 123.6L103.6 152.5Z"
                  fill="#E4761B"
                  stroke="#E4761B"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M214.9 152.5L175.9 122.9L174.6 184.2L230.7 180.9L214.9 152.5Z"
                  fill="#E4761B"
                  stroke="#E4761B"
                  strokeLinecap="round"
                  strokeLinejoin="round"
        ...
        {transactionDetails.upgradeAccount && (
          <div className="mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
            ...
            <div className="mb-2">
              ...
              <span className="text-xs font-mono ml-2 text-gray-800 break-all">
        ...
        <div className="flex justify-between mb-3">
          ...
          <div className="text-right">
            <div className="text-sm font-medium text-gray-900">
              {transactionDetails.estimatedFee.usd}
            </div>
        ...
        {Array.isArray(transactionDetails.data) ? (
          ...
          <div className="mb-4">
            ...
            >
              View details{" "}
              <FaChevronDown
                className={`ml-1 text-xs transition-transform duration-200 ${showTransactionDetails ? "transform rotate-180" : ""}`}
              />
            </button>
            ...
            {showTransactionDetails && (
              <div className="mt-3 p-3 bg-gray-50 rounded-md border border-gray-200">
                ...
                {transactionDetails.params &&
                  transactionDetails.params.length > 0 && (
                    <div className="mb-2">
      ...
      <>
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-100 rounded-md">
          ...
          <div className="text-sm text-gray-700">
            From:{" "}
            <span className="font-medium">{signatureDetails.requestFrom}</span>
          </div>
        </div>
        ...
        <div className="text-xs text-gray-500 mb-4">
          <p>
            This signature will not send a transaction or cost any gas fees.
          </p>
        </div>
      </>
    );
  ...
  const renderPrimaryWallet = () => {
    return (
      ...
      >
        <div
          className={`w-full ${walletType === "trezor" ? "max-w-lg" : "max-w-md"} bg-white rounded-xl shadow-xl flex flex-col max-h-[90vh]`}
        >
          ...
          {walletType === "trezor" && isTransaction ? (
            <div className="overflow-y-auto">{renderTrezorTransactionUI()}</div>
          ) : walletType === "trezor" && isSignature ? (
            <div className="overflow-y-auto">{renderTrezorSignatureUI()}</div>
          ) : (
            <div className="flex flex-col overflow-hidden">
              <div className="p-4 flex-1 overflow-y-auto">
                ...
                {isTransaction &&
                  renderTransactionWallet(
                    transactionOrSignatureDetails as TransactionDetails,
                  )}
                {isSignature &&
                  renderSignatureWallet(
                    transactionOrSignatureDetails as SignatureDetails,
                  )}
              </div>
