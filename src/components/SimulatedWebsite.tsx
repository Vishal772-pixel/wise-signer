...
if (!componentCache.has("defaultsite")) {
  // Handle potential variations in type name
  componentCache.set("defaultsite", DefaultSiteComponent);
}

// Helper function to load components
const loadWebsiteComponent = async (
  type: FakeWebsiteType,
): Promise<ComponentType<DynamicWebsiteProps>> => {
    ...
    switch (lowerType) {
      ...
      case "sendwethsafewallet":
        importedModule = await import(
          "@/components/fakeWebsites/SendWethSafeWallet"
        );
        break;
      case "securitycouncil":
        importedModule = await import(
          "@/components/fakeWebsites/SecurityCouncil"
        );
        break;
      case "sendtoken":
...
export default function SimulatedWebsite({
  ...
  const [CurrentDisplayComponent, setCurrentDisplayComponent] = useState<
    ComponentType<DynamicWebsiteProps>
  >(() => DefaultSiteComponent);
  ...
  useEffect(() => {
    ...
    if (fakeWebsiteType !== loadingTypeRef.current) {
      ...
      const targetCachedComponent = componentCache.get(
        fakeWebsiteType.toLowerCase(),
      );
      if (
        targetCachedComponent &&
        CurrentDisplayComponent === targetCachedComponent
      ) {
